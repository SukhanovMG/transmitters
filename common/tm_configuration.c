...
ConfigurationStatus tm_configuration_configure(void)
{
	...
	TM_LOG_TRACE("cp: %d; pool: %d; refcnt_opt: %d; refcnt_mutex: %d; q: %s; libev: %d; return: %d", configuration.copy_block_on_transfer, configuration.use_mempool, configuration.optimize_refcount_use_by_copy, configuration.refcount_with_mutex, configuration.use_rbuf_instead_of_list ? "rbuf" : "list", configuration.use_libev, configuration.return_pointers_through_pipes);
